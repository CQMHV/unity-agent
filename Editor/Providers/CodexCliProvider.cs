...
The Unity Editor host application reads your text output, detects the `<tool>...</tool>` block, executes the corresponding C# method inside the Unity Editor, and feeds the result back to you in the next message. Put each argument's value RAW between `<arg name=""..."">` and `</arg>` — do NOT escape anything (write `<`, `>`, `&`, quotes, newlines, code verbatim).
